"""
한국투자증권 KIS OpenAPI — Vercel Serverless (FastAPI)
프론트엔드 HTML + 백엔드 API 통합
"""

import os, httpx, asyncio, logging
from datetime import datetime, timedelta
from fastapi import FastAPI, Query
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import HTMLResponse, JSONResponse

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = FastAPI()
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

APP_KEY    = os.environ.get("KIS_APP_KEY", "")
APP_SECRET = os.environ.get("KIS_APP_SECRET", "")
KIS_BASE   = "https://openapi.koreainvestment.com:9443"

_token_cache: dict = {}
_data_cache:  dict = {}

# ── 에러 응답 (항상 JSON 반환) ────────────────────────────────────
def err(msg: str, detail: str = "") -> JSONResponse:
    return JSONResponse({"error": True, "msg": msg, "detail": detail, "items": []}, status_code=200)

# ── 토큰 발급 ─────────────────────────────────────────────────────
async def get_token() -> str:
    now = datetime.now()
    if _token_cache.get("token") and _token_cache.get("exp", now) > now:
        return _token_cache["token"]
    if not APP_KEY or not APP_SECRET:
        raise ValueError("KIS_APP_KEY / KIS_APP_SECRET 환경변수 미설정 — Vercel Environment Variables 확인")
    async with httpx.AsyncClient(timeout=15) as client:
        res = await client.post(
            f"{KIS_BASE}/oauth2/tokenP",
            json={"grant_type": "client_credentials", "appkey": APP_KEY, "appsecret": APP_SECRET},
            headers={"content-type": "application/json"},
        )
    data = res.json()
    logger.info(f"토큰응답 rt_cd={data.get('rt_cd')} msg={data.get('msg1','')[:60]}")
    token = data.get("access_token", "")
    if not token:
        raise ValueError(f"토큰 발급 실패: {data.get('msg1', '알 수 없는 오류')}")
    _token_cache["token"] = token
    _token_cache["exp"]   = now + timedelta(hours=23)
    return token

def make_headers(tr_id: str) -> dict:
    return {
        "content-type": "application/json",
        "authorization": f"Bearer {_token_cache.get('token','')}",
        "appkey": APP_KEY,
        "appsecret": APP_SECRET,
        "tr_id": tr_id,
        "custtype": "P",
    }

async def kis_get(path: str, params: dict, tr_id: str) -> dict:
    await get_token()
    async with httpx.AsyncClient(timeout=20) as client:
        res = await client.get(f"{KIS_BASE}{path}", params=params, headers=make_headers(tr_id))
    data = res.json()
    logger.info(f"KIS {tr_id} rt_cd={data.get('rt_cd')} msg={data.get('msg1','')[:60]}")
    return data

# ── 유틸 ──────────────────────────────────────────────────────────
def get_today() -> str:
    now = datetime.now()
    wd  = now.weekday()
    if wd == 5: now -= timedelta(days=1)
    elif wd == 6: now -= timedelta(days=2)
    if now.hour < 9:
        now -= timedelta(days=1)
        if now.weekday() == 5: now -= timedelta(days=1)
        elif now.weekday() == 6: now -= timedelta(days=2)
    return now.strftime("%Y%m%d")

def si(v) -> int:
    try: return int(v or 0)
    except: return 0

def sf(v) -> float:
    try: return float(v or 0)
    except: return 0.0

def parse_rank(raw: dict, market: str, inv_type: str) -> dict:
    output = raw.get("output", [])
    if not output:
        return {"market": market, "type": inv_type, "items": [],
                "rt_cd": raw.get("rt_cd",""), "msg": raw.get("msg1","")}
    items = []
    for i, x in enumerate(output[:10]):
        items.append({
            "rank": i+1,
            "code": x.get("mksc_shrn_iscd",""),
            "name": x.get("hts_kor_isnm",""),
            "price": si(x.get("stck_prpr")),
            "change_rate": sf(x.get("prdy_ctrt")),
            "foreign_net_amt": si(x.get("frgn_ntby_tr_pbmn")),
            "foreign_net_vol": si(x.get("frgn_ntby_qty")),
            "inst_net_amt":    si(x.get("orgn_ntby_tr_pbmn")),
            "inst_net_vol":    si(x.get("orgn_ntby_qty")),
            "retail_net_vol":  si(x.get("indv_ntby_qty")),
        })
    return {"market": market, "type": inv_type, "items": items,
            "updated": datetime.now().isoformat(),
            "rt_cd": raw.get("rt_cd",""), "msg": raw.get("msg1","")}

# ── API ───────────────────────────────────────────────────────────

@app.get("/api/health")
async def health():
    return {"status": "ok", "time": datetime.now().isoformat(),
            "key_set": bool(APP_KEY), "today": get_today()}

@app.get("/api/token-test")
async def token_test():
    try:
        t = await get_token()
        return {"success": True, "preview": t[:20]+"..."}
    except Exception as e:
        return {"success": False, "error": str(e)}

@app.get("/api/rank/foreign")
async def rank_foreign(market: str = Query("J")):
    try:
        raw = await kis_get(
            "/uapi/domestic-stock/v1/quotations/foreign-institution-total",
            {"fid_cond_mrkt_div_code": market, "fid_cond_scr_div_code": "16449",
             "fid_input_iscd": "0000", "fid_div_cls_code": "0",
             "fid_rank_sort_cls_code": "0", "fid_etc_cls_code": "0"},
            "FHPTJ04430000",
        )
        return parse_rank(raw, market, "foreign")
    except Exception as e:
        logger.error(f"rank/foreign: {e}")
        return err("외국인 순위 조회 실패", str(e))

@app.get("/api/rank/institution")
async def rank_institution(market: str = Query("J")):
    try:
        raw = await kis_get(
            "/uapi/domestic-stock/v1/quotations/foreign-institution-total",
            {"fid_cond_mrkt_div_code": market, "fid_cond_scr_div_code": "16449",
             "fid_input_iscd": "0000", "fid_div_cls_code": "1",
             "fid_rank_sort_cls_code": "0", "fid_etc_cls_code": "0"},
            "FHPTJ04400000",
        )
        return parse_rank(raw, market, "institution")
    except Exception as e:
        logger.error(f"rank/institution: {e}")
        return err("기관 순위 조회 실패", str(e))

@app.get("/api/stock-search")
async def stock_search(code: str = Query(...), market: str = Query("J")):
    try:
        d = get_today()
        p, inv = await asyncio.gather(
            kis_get("/uapi/domestic-stock/v1/quotations/inquire-price",
                    {"fid_cond_mrkt_div_code": market, "fid_input_iscd": code},
                    "FHKST01010100"),
            kis_get("/uapi/domestic-stock/v1/quotations/inquire-investor",
                    {"fid_cond_mrkt_div_code": market, "fid_input_iscd": code,
                     "fid_input_date_1": d, "fid_input_date_2": d},
                    "FHKST01010300"),
        )
        o  = p.get("output", {})
        iv_list = inv.get("output", [])
        iv = iv_list[0] if iv_list else {}
        return {
            "code": code,
            "name": o.get("hts_kor_isnm",""),
            "price": si(o.get("stck_prpr")),
            "change_rate": sf(o.get("prdy_ctrt")),
            "volume": si(o.get("acml_vol")),
            "foreign_net_amt": si(iv.get("frgn_ntby_tr_pbmn")),
            "foreign_net_vol": si(iv.get("frgn_ntby_qty")),
            "inst_net_amt":    si(iv.get("orgn_ntby_tr_pbmn")),
            "inst_net_vol":    si(iv.get("orgn_ntby_qty")),
            "retail_net_amt":  si(iv.get("indv_ntby_tr_pbmn")),
            "retail_net_vol":  si(iv.get("indv_ntby_qty")),
            "rt_cd": p.get("rt_cd",""), "msg": p.get("msg1",""),
        }
    except Exception as e:
        logger.error(f"stock-search: {e}")
        return err("종목 조회 실패", str(e))

# ── 프론트엔드 (HTML 인라인) ──────────────────────────────────────
@app.get("/", response_class=HTMLResponse)
async def frontend():
    return HTML_PAGE

HTML_PAGE = r"""<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"/>
<meta name="theme-color" content="#178FE0"/>
<meta name="apple-mobile-web-app-capable" content="yes"/>
<title>매매동향</title>
<style>
*{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
body{font-family:'Pretendard','Apple SD Gothic Neo','Malgun Gothic',sans-serif;background:#F8F8F6;min-height:100vh;font-size:14px;color:#1a1a1a}
.app{max-width:480px;margin:0 auto;background:#F8F8F6;min-height:100vh}
.hdr{background:#fff;border-bottom:.5px solid #E0E0DA;padding:14px 16px 0;position:sticky;top:0;z-index:10}
.hdr-row{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}
.ttl{font-size:18px;font-weight:500}
.live{background:#E24B4A;color:#fff;font-size:10px;font-weight:600;padding:2px 7px;border-radius:99px;margin-left:8px}
.rbtn{background:none;border:.5px solid #C8C8C0;border-radius:8px;padding:5px 10px;font-size:12px;color:#666;cursor:pointer;font-family:inherit}
.tabs{display:flex}
.tab{flex:1;text-align:center;padding:8px 0;font-size:14px;color:#888;background:none;border:none;border-bottom:2px solid transparent;cursor:pointer;font-family:inherit}
.tab.on{color:#178FE0;border-bottom:2px solid #178FE0;font-weight:500}
.stabs{display:flex;gap:8px;padding:10px 16px;background:#fff;border-bottom:.5px solid #E0E0DA}
.stab{padding:6px 16px;font-size:13px;border-radius:99px;border:.5px solid #E0E0DA;cursor:pointer;background:none;color:#666;font-family:inherit}
.stab.on{background:#E6F1FB;color:#185FA5;border-color:#B5D4F4;font-weight:500}
.filters{display:flex;gap:8px;padding:10px 16px 4px}
.fbtn{padding:5px 14px;font-size:12px;border-radius:99px;border:.5px solid #E0E0DA;cursor:pointer;background:none;color:#666;font-family:inherit}
.fbtn.on{background:#E6F1FB;color:#185FA5;border-color:#B5D4F4;font-weight:500}
.drow{padding:6px 16px;font-size:12px;color:#999}
.list{padding:0 16px 16px}
.stitle{font-size:13px;font-weight:500;color:#666;margin:8px 0}
.card{background:#fff;border:.5px solid #E0E0DA;border-radius:10px;padding:11px 13px;margin-bottom:7px;cursor:pointer}
.card.sel{border-color:#B5D4F4;background:#F5F9FF}
.ctop{display:flex;align-items:center;gap:8px}
.rnk{width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;flex-shrink:0}
.sname{font-size:14px;font-weight:500;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.scode{font-size:11px;color:#bbb;flex-shrink:0}
.namt{font-size:14px;font-weight:500;flex-shrink:0}
.brow{display:flex;align-items:center;gap:6px;margin-top:6px}
.bbg{flex:1;height:5px;background:#F0F0EC;border-radius:99px;overflow:hidden}
.bfill{height:100%;border-radius:99px;min-width:2px}
.rchg{font-size:11px;min-width:52px;text-align:right}
.dtl{margin-top:10px;padding-top:10px;border-top:.5px solid #F0F0EC}
.dgrid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:8px}
.dchip{text-align:center;padding:7px 4px;background:#F8F8F6;border-radius:6px;border:.5px solid #E0E0DA}
.dchip span{display:block;font-size:10px;color:#888;margin-bottom:2px}
.dchip b{font-size:12px;font-weight:600}
.dprice{display:flex;justify-content:space-between;font-size:13px;margin-top:6px}
.sarea{padding:12px 16px 0}
.srow{display:flex;gap:8px;margin-bottom:10px}
.sinp{flex:1;padding:9px 12px;border:.5px solid #C8C8C0;border-radius:8px;font-size:14px;background:#F8F8F6;outline:none;font-family:inherit}
.sinp:focus{border-color:#B5D4F4;background:#fff}
.sbtn{padding:9px 18px;background:#178FE0;color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit}
.qrow{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px}
.qbtn{padding:4px 10px;font-size:12px;border:.5px solid #E0E0DA;border-radius:99px;background:none;cursor:pointer;color:#444;font-family:inherit}
.rcard{background:#fff;border:.5px solid #E0E0DA;border-radius:12px;padding:14px;margin:0 16px}
.rchdr{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px}
.rname{font-size:16px;font-weight:600}
.rcode2{font-size:12px;color:#bbb;margin-top:2px}
.rprice{font-size:18px;font-weight:600}
.divdr{height:.5px;background:#F0F0EC;margin:10px 0}
.igrid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px}
.ibox{text-align:center;padding:10px 4px;background:#F8F8F6;border-radius:8px;border:.5px solid #E0E0DA}
.ilbl{font-size:11px;color:#888;margin-bottom:4px}
.iamt{font-size:13px;font-weight:600;margin-bottom:2px}
.ivol{font-size:10px;color:#bbb}
.empty{text-align:center;padding:40px 16px;color:#bbb;font-size:14px}
.loading{text-align:center;padding:32px;color:#888;font-size:14px}
.errmsg{margin:8px 16px;padding:12px;background:#FFF0F0;border:.5px solid #F7C1C1;border-radius:8px;font-size:13px;color:#A32D2D;line-height:1.6}
.apimsg{margin:8px 16px;padding:10px 12px;background:#FFFBE6;border:.5px solid #F0D070;border-radius:8px;font-size:12px;color:#7A5A00}
.page{display:none}.page.on{display:block}
.footer{text-align:center;padding:20px 16px;font-size:11px;color:#ccc;border-top:.5px solid #E8E8E4;margin-top:8px}
</style>
</head>
<body>
<div class="app">
  <div class="hdr">
    <div class="hdr-row">
      <div style="display:flex;align-items:center">
        <span class="ttl">매매동향</span><span class="live">LIVE</span>
      </div>
      <button class="rbtn" id="refreshBtn" onclick="doRefresh()">↺ 새로고침</button>
    </div>
    <div class="tabs">
      <button class="tab on" onclick="switchTab('rank',this)">순위</button>
      <button class="tab" onclick="switchTab('search',this)">종목검색</button>
    </div>
  </div>

  <div id="pg-rank" class="page on">
    <div class="stabs" id="market-stabs">
      <button class="stab on" onclick="switchMarket('J',this)">코스피</button>
      <button class="stab" onclick="switchMarket('Q',this)">코스닥</button>
    </div>
    <div class="filters">
      <button class="fbtn on" onclick="switchInv('foreign',this)">외국인</button>
      <button class="fbtn" onclick="switchInv('institution',this)">기관</button>
    </div>
    <div class="drow" id="dateLabel">기준일: 로딩 중...</div>
    <div id="apiMsg" style="display:none"></div>
    <div class="list">
      <div class="stitle" id="rankTitle">외국인 순매수 상위 10 · 코스피</div>
      <div id="rankList"><div class="loading">데이터 불러오는 중...</div></div>
    </div>
  </div>

  <div id="pg-search" class="page">
    <div class="sarea">
      <div class="stabs" id="search-stabs" style="padding:0 0 12px;border:none;background:transparent">
        <button class="stab on" onclick="switchSMkt('J',this)">코스피</button>
        <button class="stab" onclick="switchSMkt('Q',this)">코스닥</button>
      </div>
      <div class="srow">
        <input class="sinp" id="searchInput" type="text" placeholder="종목코드 6자리 (예: 005930)"
          maxlength="6" onkeydown="if(event.key==='Enter')doSearch()"/>
        <button class="sbtn" onclick="doSearch()">조회</button>
      </div>
      <div class="qrow">
        <button class="qbtn" onclick="quick('005930','J')">삼성전자</button>
        <button class="qbtn" onclick="quick('000660','J')">SK하이닉스</button>
        <button class="qbtn" onclick="quick('035420','J')">NAVER</button>
        <button class="qbtn" onclick="quick('005380','J')">현대차</button>
        <button class="qbtn" onclick="quick('247540','Q')">에코프로비엠</button>
        <button class="qbtn" onclick="quick('196170','Q')">알테오젠</button>
      </div>
    </div>
    <div id="searchResult"></div>
  </div>

  <div class="footer">한국투자증권 KIS OpenAPI · 장중 60초 자동갱신</div>
</div>

<script>
let market='J',inv='foreign',sMkt='J',selCode=null,rankData=null;

function switchTab(name,el){
  document.querySelectorAll('.tab').forEach(t=>t.classList.remove('on'));
  el.classList.add('on');
  document.querySelectorAll('.page').forEach(p=>p.classList.remove('on'));
  document.getElementById('pg-'+name).classList.add('on');
}
function switchMarket(m,el){market=m;document.querySelectorAll('#market-stabs .stab').forEach(b=>b.classList.remove('on'));el.classList.add('on');loadRank();}
function switchInv(i,el){inv=i;document.querySelectorAll('.fbtn').forEach(b=>b.classList.remove('on'));el.classList.add('on');loadRank();}
function switchSMkt(m,el){sMkt=m;document.querySelectorAll('#search-stabs .stab').forEach(b=>b.classList.remove('on'));el.classList.add('on');}

const fmtAmt=v=>{const n=Math.round(Number(v)/100000000);return(n>=0?'+':'')+n.toLocaleString()+'억';};
const fmtRate=v=>{const n=Number(v);return(n>=0?'+':'')+n.toFixed(2)+'%';};
const fmtPrice=v=>Number(v).toLocaleString()+'원';
const colorOf=v=>Number(v)>0?'#E24B4A':Number(v)<0?'#185FA5':'#888';
const rnkStyle=r=>r===1?'background:#FAC775;color:#633806':r===2?'background:#D3D1C7;color:#444':r===3?'background:#F0997B;color:#4A1B0C':'background:#EEECEA;color:#666';

async function loadRank(){
  document.getElementById('rankList').innerHTML='<div class="loading">데이터 불러오는 중...</div>';
  document.getElementById('apiMsg').style.display='none';
  selCode=null;
  const ep=inv==='foreign'?`/api/rank/foreign?market=${market}`:`/api/rank/institution?market=${market}`;
  try{
    const res=await fetch(ep);
    const text=await res.text();
    let data;
    try{data=JSON.parse(text);}
    catch(e){
      showMsg('error','서버 응답 오류 — 잠시 후 다시 시도해 주세요<br><small style="opacity:.7">'+text.substring(0,120)+'</small>');
      document.getElementById('rankList').innerHTML='';return;
    }
    if(data.error){
      showMsg('error',data.msg+(data.detail?'<br><small style="opacity:.7">'+data.detail+'</small>':''));
      document.getElementById('rankList').innerHTML='';return;
    }
    if(data.rt_cd&&data.rt_cd!=='0'){
      showMsg('warn','KIS API 메시지: '+data.msg+' (rt_cd='+data.rt_cd+')');
    }
    rankData=data;renderRank(data);
    const now=new Date();
    document.getElementById('dateLabel').textContent=
      `기준: ${now.getFullYear()}.${String(now.getMonth()+1).padStart(2,'0')}.${String(now.getDate()).padStart(2,'0')} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
  }catch(e){showMsg('error','네트워크 오류: '+e.message);document.getElementById('rankList').innerHTML='';}
  const mk=market==='J'?'코스피':'코스닥',il=inv==='foreign'?'외국인':'기관';
  document.getElementById('rankTitle').textContent=`${il} 순매수 상위 10 · ${mk}`;
}

function showMsg(type,html){
  const el=document.getElementById('apiMsg');
  el.style.display='block';
  el.className=type==='error'?'errmsg':'apimsg';
  el.innerHTML=(type==='error'?'⚠️ ':'ℹ️ ')+html;
}

function renderRank(data){
  const items=data.items||[];
  if(!items.length){document.getElementById('rankList').innerHTML='<div class="empty">데이터 없음 (장 마감 / 휴장일)</div>';return;}
  const maxAmt=Math.max(...items.map(x=>Math.abs(inv==='foreign'?x.foreign_net_amt:x.inst_net_amt)),1);
  document.getElementById('rankList').innerHTML=items.map((item,i)=>{
    const netAmt=inv==='foreign'?item.foreign_net_amt:item.inst_net_amt;
    const pct=Math.min(100,Math.round(Math.abs(netAmt)/maxAmt*100));
    const barCol=inv==='foreign'?'#E24B4A':'#178FE0';
    const r=i+1;
    return `<div class="card${selCode===item.code?' sel':''}" onclick="toggleDetail('${item.code}')">
      <div class="ctop">
        <span class="rnk" style="${rnkStyle(r)}">${r}</span>
        <span class="sname">${item.name||item.code}</span>
        <span class="scode">${item.code}</span>
        <span class="namt" style="color:${colorOf(netAmt)}">${fmtAmt(netAmt)}</span>
      </div>
      <div class="brow">
        <div class="bbg"><div class="bfill" style="width:${pct}%;background:${barCol}"></div></div>
        <span class="rchg" style="color:${colorOf(item.change_rate)}">${fmtRate(item.change_rate)}</span>
      </div>
      ${selCode===item.code?detailHtml(item):''}
    </div>`;
  }).join('');
}

function toggleDetail(code){selCode=selCode===code?null:code;if(rankData)renderRank(rankData);}

function detailHtml(item){
  const retail=-(item.foreign_net_amt+item.inst_net_amt);
  return `<div class="dtl">
    <div class="dgrid">
      <div class="dchip"><span>외국인</span><b style="color:${colorOf(item.foreign_net_amt)}">${fmtAmt(item.foreign_net_amt)}</b></div>
      <div class="dchip"><span>기관</span><b style="color:${colorOf(item.inst_net_amt)}">${fmtAmt(item.inst_net_amt)}</b></div>
      <div class="dchip"><span>개인(추정)</span><b style="color:${colorOf(retail)}">${fmtAmt(retail)}</b></div>
    </div>
    <div class="dprice"><span style="color:#888">현재가</span><span style="font-weight:600;color:${colorOf(item.change_rate)}">${fmtPrice(item.price)}</span></div>
  </div>`;
}

function doRefresh(){
  const btn=document.getElementById('refreshBtn');
  btn.textContent='⟳ 갱신 중...';btn.disabled=true;
  loadRank().finally(()=>{btn.textContent='↺ 새로고침';btn.disabled=false;});
}

async function doSearch(){
  const code=document.getElementById('searchInput').value.trim();
  if(!code)return;
  const res=document.getElementById('searchResult');
  res.innerHTML='<div class="loading">조회 중...</div>';
  try{
    const r=await fetch(`/api/stock-search?code=${code}&market=${sMkt}`);
    const text=await r.text();
    let data;
    try{data=JSON.parse(text);}
    catch(e){res.innerHTML=`<div class="errmsg">⚠️ 서버 오류: ${text.substring(0,120)}</div>`;return;}
    if(data.error){res.innerHTML=`<div class="errmsg">⚠️ ${data.msg}${data.detail?' — '+data.detail:''}</div>`;return;}
    if(!data.name&&!data.price){res.innerHTML='<div class="errmsg">⚠️ 종목을 찾을 수 없습니다. 코드와 시장(코스피/코스닥) 확인</div>';return;}
    res.innerHTML=`<div class="rcard">
      <div class="rchdr">
        <div><div class="rname">${data.name||code}</div><div class="rcode2">${code} · ${sMkt==='J'?'코스피':'코스닥'}</div></div>
        <div style="text-align:right">
          <div class="rprice" style="color:${colorOf(data.change_rate)}">${fmtPrice(data.price)}</div>
          <div style="font-size:12px;color:${colorOf(data.change_rate)};margin-top:2px">${fmtRate(data.change_rate)}</div>
        </div>
      </div>
      <div class="divdr"></div>
      <div class="igrid">
        ${[['외국인',data.foreign_net_amt,data.foreign_net_vol],
           ['기관합계',data.inst_net_amt,data.inst_net_vol],
           ['개인',data.retail_net_amt,data.retail_net_vol]].map(([lbl,amt,vol])=>`
          <div class="ibox">
            <div class="ilbl">${lbl}</div>
            <div class="iamt" style="color:${colorOf(amt)}">${fmtAmt(amt||0)}</div>
            <div class="ivol">${Number(vol||0).toLocaleString()}주</div>
          </div>`).join('')}
      </div>
      <div class="divdr"></div>
      <div style="font-size:13px"><div style="font-size:11px;color:#888;margin-bottom:3px">거래량</div><b>${Number(data.volume||0).toLocaleString()}</b></div>
      ${data.rt_cd&&data.rt_cd!=='0'?`<div class="apimsg" style="margin-top:10px">ℹ️ ${data.msg}</div>`:''}
    </div>`;
  }catch(e){res.innerHTML=`<div class="errmsg">⚠️ 네트워크 오류: ${e.message}</div>`;}
}

function quick(code,mkt){
  sMkt=mkt;
  document.querySelectorAll('#search-stabs .stab').forEach((b,i)=>b.classList.toggle('on',['J','Q'][i]===mkt));
  document.getElementById('searchInput').value=code;doSearch();
}

loadRank();
setInterval(loadRank,60000);
</script>
</body>
</html>"""
